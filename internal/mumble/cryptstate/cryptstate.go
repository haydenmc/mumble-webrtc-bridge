...
//
// Trimmed from the upstream package for this project: only the OCB2-AES128
// mode is retained (the only mode the mainline Mumble/Murmur server speaks),
// and a SetDecryptIV helper was added to support the client-side crypt
// resync handshake. See ../NOTICE.md.
...
// ModeOCB2AES128 is the only CryptoMode this package implements, matching
// the mainline Mumble/Murmur server.
const ModeOCB2AES128 = "OCB2-AES128"
...
// SetDecryptIV replaces the current decrypt IV in place, used to resync the
// decrypt direction when the server sends us its current EncryptIV (a
// CryptSetup message containing only server_nonce). Not part of upstream
// Grumble, which only ever plays the server role.
func (cs *CryptState) SetDecryptIV(iv []byte) error {
	if cs.mode == nil {
		return errors.New("cryptstate: crypt state has no mode set")
	}
	if len(iv) != len(cs.DecryptIV) {
		return errors.New("cryptstate: invalid decrypt IV length")
	}
